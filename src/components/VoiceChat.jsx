...
async function fetchIceServers() {
  try {
    ...
    const res = await fetch(`${API_URL}/game/turn-credentials`, {
    ...
    // Fallback to STUN-only if endpoint unreachable
...
const VoiceChat = ({ gameId, userId, players }) => {
  ...
  const createPeerConnection = () => {
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) {
        socket.emit('voiceCandidate', { gameId, senderId: userId, candidate: event.candidate });
      }
    };
  ...
  const startVoice = async () => {
    ...
    try {
      ...
      // Fetch TURN credentials from backend if not already cached
      ...
      socket.emit('voiceOffer', { gameId, senderId: userId, offer: pc.localDescription });
    } catch (err) {
  ...
  const handleVoiceOffer = async (data) => {
    ...
    try {
      if (isCollision) await pc.setLocalDescription({ type: 'rollback' });
      await pc.setRemoteDescription(new RTCSessionDescription(data.offer));
      await flushCandidates();
      const answer = await pc.createAnswer();
      await pc.setLocalDescription(answer);
      socket.emit('voiceAnswer', { gameId, senderId: userId, answer: pc.localDescription });
  ...
  return (
    <div className="flex flex-col items-end gap-3">
      <div className="flex flex-col gap-2">
        ...
        <button
          onClick={toggleVoice}
          className={`relative flex h-11 w-11 items-center justify-center rounded-full border text-white shadow-2xl transition-all ${
            isActive ? 'bg-emerald-500/90 border-emerald-300' : 'bg-slate-950/90 border-white/10 hover:bg-slate-900'
          }`}
      ...
      {isActive && isMenuOpen && (
        <div className="w-[min(100vw-2rem,300px)] rounded-3xl border border-white/10 bg-slate-950/95 px-4 py-3 text-white shadow-2xl">
          ...
          <div className="mt-3 text-[11px] text-slate-400">
            {error ? error : remoteConnected ? 'Audio linked successfully.' : 'Waiting for opponent audio...'}
          </div>
