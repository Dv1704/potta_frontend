...
export default function GameDashboard() {
  ...
  const startMatchmaking = () => {
    ...
    if (!amount || amount < 10) {
      showToast('Min entry fee is GHC 10', 'error');
      return;
    }
    ...
    const mode = selectedGame?.mode || 'turn';
    navigate(`/quick-match?mode=${mode}&entryFee=${amount}`);
  };
      ...
      <AnimatePresence>
        {showEntryFeeModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowEntryFeeModal(false)} className="absolute inset-0 bg-black/90 backdrop-blur-md" />
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="bg-[#0c111d] w-full max-w-md rounded-[3rem] border border-white/10 overflow-hidden shadow-2xl relative z-10">
              <div className="p-12">
                <div className="flex justify-between items-center mb-10">
                  <div>
                    <h3 className="text-3xl font-black uppercase tracking-tighter italic">Set Entry Fee</h3>
                    <p className="text-gray-400 text-sm mt-2">{selectedGame?.name || 'Select a game'} • Min GH₵{selectedGame?.minEntry || 10}</p>
                  </div>
                  <button onClick={() => setShowEntryFeeModal(false)} className="w-10 h-10 bg-white/5 rounded-full flex items-center justify-center text-gray-500 hover:text-white transition-all"><X size={24} /></button>
                </div>
                <div className="bg-black/40 rounded-3xl p-8 border border-white/5 mb-8 text-center">
                  <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest mb-2">ENTRY FEE AMOUNT</p>
                ...
                <div className="grid grid-cols-4 gap-4 mb-10">
                  {[10, 20, 50, 100].map((amount) => (
                    <button key={amount} onClick={() => setEntryFee(amount.toString())} className="py-4 bg-white/5 hover:bg-blue-600 rounded-xl font-black transition-all border border-white/5 active:scale-90">GH₵{amount}</button>
                  ))}
                </div>
      ...
      <AnimatePresence>
        {showDepositModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="bg-[#0c111d] w-full max-w-md rounded-[3rem] border border-white/10 overflow-hidden shadow-2xl relative z-10">
              <form onSubmit={handleDeposit} className="p-12 text-center">
                <div className="flex justify-between items-center mb-10">
                  <h3 className="text-3xl font-black uppercase tracking-tighter italic">Deposit Funds</h3>
                  <button type="button" onClick={() => setShowDepositModal(false)} className="w-10 h-10 bg-white/5 rounded-full flex items-center justify-center text-gray-500 hover:text-white transition-all"><X size={24} /></button>
                </div>
                <div className="bg-black/40 rounded-3xl p-8 border border-white/5 mb-8 text-center relative">
                  <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest mb-2">DEPOSIT AMOUNT</p>
                  <div className="flex items-center justify-center gap-2">
                    <div className="text-3xl font-bold text-blue-500">GH₵</div>
                    <input
                      type="number"
                      min="10"
                      value={depositAmount}
                      onChange={(e) => setDepositAmount(e.target.value)}
                      className="w-full bg-transparent text-5xl font-black tracking-tighter text-center outline-none border-b-2 border-transparent focus:border-blue-500 transition-all"
                      placeholder="Min 10"
                    />
                  </div>
                </div>
                <div className="flex gap-4">
                  ...
                  <button type="submit" disabled={depositLoading} className="flex-[2] bg-blue-600 py-5 px-10 rounded-2xl font-black text-white shadow-xl shadow-blue-600/20 hover:bg-blue-500 transition-all">{depositLoading ? 'PROCESSING...' : 'CONFIRM DEPOSIT'}</button>
                </div>
              </form>
            </motion.div>
          </div>
