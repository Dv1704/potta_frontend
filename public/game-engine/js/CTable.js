...
function CTable(oParentContainer, oCpuDifficultyParams) {
        ...
        this._init = function (oCpuDifficultyParams) {
                ...
                // Positioned at bottom of cloth so it doesn't overlap the table's center triangle logo
                ...
                oLogoBmp.y = oSpriteBg.height * 0.82;
                oLogoImg.onload = function () {
                    var fScale = Math.min(
                        (oSpriteBg.width * 0.32) / oLogoImg.width,
                        (oSpriteBg.height * 0.18) / oLogoImg.height
                    );
        ...
        /**
         * Executes a shot triggered by a remote player (opponent).
         */
        this.executeRemoteShot = function (data) {
                // data: { angle, power, sideSpin, backSpin }
                // console.log("[GameEngine] Executing remote shot:", data);

                // 1. Calculate direction vector from angle (degrees)
                var fRad = data.angle * Math.PI / 180;
                _vStickDirection.set(Math.cos(fRad), Math.sin(fRad));

                // 2. Set Power (Apply 0.2 scaling to match local logic)
                _iPowerShot = data.power * 0.2;

                // 3. Set Spin
                _oCueBall.setSideEffect(data.sideSpin || 0);
        ...
        this.updateBallsFromServer = function (serverBalls) {
                if (!serverBalls) return;

                // serverBalls is an OBJECT: { ballId: {x, y, onTable} }
                // x and y are percentages (0-100)

                for (var key in serverBalls) {
                        if (serverBalls.hasOwnProperty(key)) {
                                var ballId = parseInt(key);
                                ...
                                if (_aBalls[ballId]) {
                                        ...
                                        this._update3DObjectTransformation(_aBalls[ballId]);

                                        if (_aBalls[ballId]._updateShadow) {
                                                _aBalls[ballId]._updateShadow();
                                        }
                                }
                        }
                }
                ...
                if (_bHoldStick === false) {
                        if (_bReadyForShot) {
                                ...
                                if (_iTimeAnimStick >= _iMaxTimeAnimationStick) {
                                        ...
                                        // --- SYNC ADDITION ---
                                        // Notify the parent window that a shot has been taken
                                        if (typeof s_bIsMyTurn !== 'undefined' && s_bIsMyTurn) {
                                                console.log("[GameEngine] Sending shot to parent...");
                                                var fAngle = radiantsToDegrees(Math.atan2(_vStickDirection.getY(), _vStickDirection.getX()));
