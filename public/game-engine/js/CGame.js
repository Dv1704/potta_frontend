function CGame() {
    ...
    this._init = function () {
        ...
        // --- SYNC IMPLEMENTATION ---
        var oThis = this;
        this._oMessageListener = function (event) {
            if (!event.data) return;

            if (event.data.type === 'initUser') {
                s_szUserId = event.data.userId;
                console.log("[GameEngine] User initialized:", s_szUserId);
            }
    ...
    this.unload = function (oCbCompleted = null, oCbScope) {
        ...
        if (this._oMessageListener) {
            window.removeEventListener('message', this._oMessageListener);
            this._oMessageListener = null;
        }
